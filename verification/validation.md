...
- **Android unit tests and lint passed** (`./gradlew :app:testDebugUnitTest :app:lintDebug`, 352 tests, 0 failures, 0 lint errors), including the new `onlyGermanIsSelectableAndCareThemesAreTranslated`. The Android instrumented tests (`DesignReviewTest.kt`, `SettingsParityTest.kt`) still have not been executed; no emulator has been created yet.
