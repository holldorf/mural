...
- Later the same day, after installing the toolchain (JDK 17, Android SDK 36, iOS 26.4 simulator, XcodeBuildMCP): the **iPhone app built** on an `iPhone 17` (iOS 26.4) simulator and **all 23 native UI tests passed**. Two tests first failed because the everyday tile `Ein Kaffee?` now sits below the fold behind the thirteen care-work tiles; they were changed to select the `Everyday` category chip first. The onboarding screen was also checked by screenshot: only `Deutsch` is offered under “German for care work”, and the Themes tab lists `Care work` first with `Die Übergabe`.
- **Android unit tests and lint passed** (`./gradlew :app:testDebugUnitTest :app:lintDebug`, 352 tests, 0 failures, 0 lint errors), including the new `onlyGermanIsSelectableAndCareThemesAreTranslated`. The two previously unverified Android instrumented test classes now pass on the `mural36` emulator (Android 16 / API 36, default ARM64 image, Pixel 6 profile): `./gradlew --no-daemon --console=plain :app:connectedUiTestAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=chat.mural.DesignReviewTest,chat.mural.SettingsParityTest` ran **3 tests, 0 failures, 0 errors, 0 skipped**, confirmed in the JUnit XML report. This verifies these two classes only; the earlier full instrumented-suite run did not produce a completion report.
- Xcode’s package resolution hangs on this Mac when it downloads the WebRTC binary artifact through keychain-backed auth; the same file downloads in seconds with `curl`. The workaround is recorded in the root `AGENTS.md`.
